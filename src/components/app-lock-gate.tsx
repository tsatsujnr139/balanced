...
export function AppLockGate({ children }: PropsWithChildren) {
  ...
  const { capability, errorMessage, isAuthenticating, isEnabled, isHydrating, isUnlocked, unlock } = useAppLock();

  useEffect(() => {
    if (!isHydrating && isEnabled && !isUnlocked && capability?.canAuthenticate) {
      unlock();
    }
    ...
    <ScrollView
      ...
      <View className="items-center gap-5">
        <View
          ...
          <SymbolView name="lock.shield.fill" size={42} tintColor={colors.primary} />
        </View>
        ...
        <View className="items-center gap-2">
          ...
          <ThemedText className="max-w-[320px] text-center" color="muted" selectable type="small">
            {statusMessage}
          </ThemedText>
        </View>
        ...
        <Pressable
          ...
          style={{ opacity: isAuthenticating || !capability?.canAuthenticate ? 0.55 : 1 }}>
