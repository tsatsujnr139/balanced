...
import { PLANNED_OVERDUE_COLOR, PLANNED_TODAY_COLOR } from '../planned-payment-constants';
...
function dueLabelColor(payment: PlannedPayment): 'muted' | undefined {
  return payment.dueStatus === 'overdue' || payment.dueStatus === 'today' ? undefined : 'muted';
}
...
function PlannedPaymentRow({ payment }: { payment: PlannedPayment }) {
  ...
  return (
    <Pressable
      ...
      onPress={() => {
        router.push({ pathname: '/planned-payment/[id]', params: { id: payment.id } });
      }}>
      <View
      ...
      <View className="shrink-0 items-end gap-1">
        {payment.overdueCount > 0 ? <OverdueBadge count={payment.overdueCount} /> : null}
        <ThemedText type="smallBold" color={amountColor} className="text-base leading-[22px]">
          {formatCurrency(
