...
function TemplateRow({
  ...
  const amount =
    template.type === 'income' ? template.amount : template.type === 'transfer' ? 0 : -template.amount;

  return (
    <Pressable accessibilityRole="button" className="flex-row items-start gap-2.5" onPress={onPress}>
      <View
        className="size-[42px] shrink-0 items-center justify-center rounded-full"
        style={{ backgroundColor: template.color }}>
        <SymbolView name={template.symbol as never} size={18} tintColor="#fff" />
      </View>
      ...
      <View className="shrink-0 items-end gap-1">
        ...
        {template.transactionCharge ? (
          <ThemedText type="small" color="negative" style={{ fontSize: 13, fontWeight: '600' }}>
            {formatCurrency(-template.transactionCharge, template.currency, { signed: true })}
          </ThemedText>
        ) : null}
      </View>
    </Pressable>
    ...
    return (
      <ThemedView variant="card" className="items-center rounded-[22px] px-4 py-7">
        <View className="mb-3 size-11 items-center justify-center rounded-full bg-background">
          <SymbolView name="rectangle.stack.badge.plus" size={22} tintColor="#8E8E93" />
        </View>
  ...
  return (
    <ThemedView variant="card" className="rounded-[22px] p-4">
      {templates.map((template, index) => (
        <View key={template.id} className={cn(index < templates.length - 1 && 'mb-4')}>
          <TemplateRow template={template} onPress={() => onPressTemplate(template)} />
        </View>
      ))}
    </ThemedView>
  );
