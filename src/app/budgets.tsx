...
import { BudgetList, BudgetProgressRow } from '@/features/finance/components/budget-list';
...
export default function BudgetsScreen() {
  ...
  const groups = useMemo(() => groupBudgetsByPeriod(filteredBudgets), [filteredBudgets]);
      ...
      <ScrollView
        ...
        {filteredBudgets.length === 0 ? (
          ...
          groups.map(([period, periodBudgets]) => (
            <ThemedView key={period} variant="card" className="rounded-[22px] p-4">
              <ThemedText type="subtitle" className="mb-3 text-[20px] leading-7">
              ...
              {periodBudgets.map((budget, index) => (
                <View
                  key={budget.id}
                  className={index < periodBudgets.length - 1 ? 'mb-5' : undefined}>
                  <BudgetProgressRow
                    budget={budget}
                    onPress={() => {
                      router.push({ pathname: '/budget/[id]', params: { id: budget.id } });
                    }}
                  />
                </View>
              ))}
