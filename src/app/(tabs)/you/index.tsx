...
import {
  normalizeFirstName,
  useLocalProfile,
} from "@/features/finance/use-local-profile";
...
function getAuthErrorMessage(
  reason: "cancelled" | "not_available" | "not_enrolled" | "failed" | "unknown"
) {
  switch (reason) {
    case "cancelled": {
...
export default function YouScreen() {
  ...
  async function handleValueChange(nextEnabled: boolean) {
    ...
    const result = await unlock(
      nextEnabled ? "Enable App Lock" : "Disable App Lock"
    );
  ...
  return (
    ...
    >
      <View className="gap-3">
        ...
        >
          <View className="flex-row items-center gap-3 p-4">
            <View
              ...
              style={{ borderCurve: "continuous", height: 44, width: 44 }}
            >
              <SymbolView
            ...
            <Pressable
              ...
              style={({ pressed }) => ({
                ...
                minWidth: 64,
                opacity:
                  pressed || !hasProfileChanges || isSavingProfile ? 0.45 : 1,
        ...
        {profileMessage ? (
          <ThemedText
            color={profileMessage === "Profile saved." ? "positive" : "muted"}
            selectable
            type="small"
          >
            {profileMessage}
      ...
      <View className="gap-3">
        ...
        {localMessage || errorMessage || disabledReason ? (
          <ThemedText
            color={localMessage?.includes("on.") ? "positive" : "muted"}
            selectable
            type="small"
          >
            {localMessage ?? errorMessage ?? disabledReason}
