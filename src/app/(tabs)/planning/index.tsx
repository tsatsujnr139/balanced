...
function PlanningGridCard({ card, width }: { card: PlanningCard; width: number }) {
  return (
    <Pressable
      ...
      <ThemedView
        ...
        <View className="flex-row items-start justify-between">
          <View
            ...
            <SymbolView name={card.symbol as never} size={24} tintColor="#fff" />
          </View>
          {card.badge && card.badge > 0 ? (
            <View
              className="min-w-6 items-center justify-center rounded-full px-1.5 py-0.5"
              style={{ backgroundColor: '#FF3B30' }}>
              <ThemedText type="smallBold" className="text-xs" style={{ color: '#fff' }}>
        ...
        <View className="gap-1">
          <ThemedText type="subtitle" numberOfLines={1} className="text-[20px] leading-6">
            {card.title}
          </ThemedText>
